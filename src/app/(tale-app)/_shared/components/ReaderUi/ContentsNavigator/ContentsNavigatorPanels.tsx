		...
		>
			{contents.map((part, partIndex) => {
				...
				return (
					...
					>
						<div className="mb-2 flex items-center gap-1 rounded hover:bg-white/5">
							<button
								type="button"
								aria-label={
									collapsed ? `Expand ${part.title}` : `Collapse ${part.title}`
								}
								className="grid h-9 w-9 shrink-0 place-items-center text-white/48 hover:text-white"
											...
											>
												{entry.pages.length > 1 ? (
													<button
														...
														className="grid h-6 w-6 shrink-0 place-items-center rounded hover:bg-white/8"
