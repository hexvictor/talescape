...
export default function ModalBackdrop({ children }: ModalBackdropProps) {
	...
	if (hasBackdrop || (!hasBackdrop && blockInteraction)) {
		return (
			<div
				className={`modal-backdrop ${blockInteraction ? "" : "pointer-events-none"} absolute inset-0 z-[1000] flex h-screen w-screen items-center justify-center ${hasBackdrop ? "bg-black/70" : ""}`}
				ref={backdropRef}
