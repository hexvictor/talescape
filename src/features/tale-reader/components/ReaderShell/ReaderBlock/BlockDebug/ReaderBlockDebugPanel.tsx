...
function Field({
	...
	return (
		<div className="min-w-0 rounded-xl border border-white/10 bg-white/5 px-2.5 py-2">
			<p className="mb-1 font-semibold text-[10px] text-white/55 uppercase tracking-wide">
				{label}
			</p>
			<p className="break-all text-[11px] text-white leading-tight sm:text-xs">
				{value ?? "—"}
			</p>
		</div>
	);
...
function SectionCard({
	...
	return (
		<div className={`rounded-2xl border p-3 shadow-lg ${tone}`}>
			<p className="mb-2 font-bold text-[11px] uppercase tracking-wide sm:text-sm">
				{title}
			</p>
	...
	return (
		<div className="pointer-events-none absolute top-24 right-2 z-[60] flex max-w-[min(96vw,72rem)] flex-col items-end gap-2 sm:top-24 sm:right-3">
			<button
				...
				className="pointer-events-auto inline-flex h-10 min-w-10 items-center justify-center rounded-full border border-white/20 bg-black/75 px-3 font-semibold text-[11px] text-white shadow-xl backdrop-blur-md transition hover:bg-black/85 sm:h-11 sm:text-xs"
			>
				...
				<div className="pointer-events-auto max-h-[78vh] w-[min(96vw,72rem)] overflow-y-auto rounded-3xl border border-white/15 bg-black/75 p-3 shadow-2xl backdrop-blur-md sm:p-4">
					<div className="mb-3 flex items-center justify-between gap-3">
						<div className="min-w-0">
							<p className="font-bold text-[10px] text-white/60 uppercase tracking-[0.2em]">
								Block Inspector
							</p>
...
const ReaderBlockDebugPanel = React.memo(ReaderBlockDebugPanelComponent);
export default ReaderBlockDebugPanel;
