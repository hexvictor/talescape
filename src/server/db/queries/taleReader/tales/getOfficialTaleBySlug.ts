...
import { getFormattedTale } from "~/lib/tale/getFormattedTale";
import { cache } from "react";
import { auth } from "@clerk/nextjs/server";
import { taleProgresses } from "~/server/db/schema";
import { createNewProgress } from "./createNewProgress";

export async function getOfficialTaleBySlugQuery(
  slug: string
): Promise<TaleData> {
...
export const getOfficialTaleBySlug = cache(getOfficialTaleBySlugQuery);
